...
SELECT vendor_id, COUNT(*) as total_rented_booth
FROM vendor_booth_assignments
GROUP BY vendor_id;
...
SELECT c.customer_id, c.customer_first_name, c.customer_last_name,SUM(cp.cost_to_customer_per_qty)
FROM customer AS c
...
FROM customer_purchases
WHERE 
	CAST(strftime('%m', market_date) AS INTEGER) = 4
	AND CAST (strftime('%Y', market_date) AS INTEGER) = 2022
